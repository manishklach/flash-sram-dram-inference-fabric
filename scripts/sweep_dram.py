...
CSV_ARTIFACT = ARTIFACT_DIR / "dram_capacity_sweep.csv"
JSON_ARTIFACT = ARTIFACT_DIR / "dram_capacity_sweep.json"
...
def main() -> None:
    ...
    dram_capacity_mb_values = [16, 32, 64, 128, 256, 512]
    modes = [
    ...
    print(
        "workload,mode,dram_capacity_mb,p95_us,p99_us,seq_ratio,sync_failures,dram_evictions,prefetch_waste_rate"
    )

    rows: list[dict[str, object]] = []
    for dram_capacity_mb in dram_capacity_mb_values:
        config = SimulatorConfig(dram_capacity_bytes=dram_capacity_mb * 1_048_576)
